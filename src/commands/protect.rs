...
pub async fn cameras_show(
    ...
    println!(
        "  {}  {}",
        label("Mic:       "),
        if c.is_mic_enabled { "enabled" } else { "disabled" }
    );
...
pub async fn rtsps_delete(
    ...
    let camera_id = client.resolve_camera_id(id_or_name).await?;
    client
        .delete_rtsps_streams(&camera_id, qualities)
        .await?;
    ...
    out.print_result(
        ...
        &format!("Deleted RTSPS streams ({}) for camera {id_or_name}", qualities.join(", ")),
    );
    Ok(())
}
...
pub async fn cameras_list_full(
    ...
    if out.json {
        ...
        let types: Vec<&str> = cameras
            .iter()
            .map(|c| c.market_name.as_deref().or(c.camera_type.as_deref()).unwrap_or("-"))
            .collect();
        let name_w = col(4, 4, names.iter().map(|n| n.len()).collect());
        ...
        for c in &cameras {
            ...
            if color {
                ...
                println!(
                    " {:<nw$} {:<tw$} {:<16} {:<12} {:<10} {:<12} {}",
                    name.bold(), ctype, ip, state_display, fw, rec, wifi,
                    nw = name_w - 1, tw = type_w,
                );
            } else {
                println!(
                    " {:<nw$} {:<tw$} {:<16} {:<12} {:<10} {:<12} {}",
                    name, ctype, ip, state, fw, rec, wifi,
                    nw = name_w - 1, tw = type_w,
                );
            }
        }
...
pub async fn cameras_show_full(
    ...
    println!(
        "  {}  {}",
        label("MAC:       "),
        c.mac.as_deref().map(format_mac).unwrap_or_else(|| "-".into())
    );
    println!("  {}  {}", label("IP:        "), c.host.as_deref().unwrap_or("-"));
    println!("  {}  {}", label("State:     "), c.state.as_deref().unwrap_or("-"));
    println!(
        "  {}  {}",
        label("Type:      "),
        c.market_name.as_deref().or(c.camera_type.as_deref()).unwrap_or("-")
    );
    println!("  {}  {}", label("Firmware:  "), c.firmware_version.as_deref().unwrap_or("-"));
    if let Some(uptime) = c.uptime {
        println!("  {}  {}", label("Uptime:    "), format_uptime(uptime));
    }
    println!("  {}  {}", label("Codec:     "), c.video_codec.as_deref().unwrap_or("-"));
    println!("  {}  {}", label("Resolution:"), c.current_resolution.as_deref().unwrap_or("-"));
    println!("  {}  {}", label("HDR:       "), c.hdr_type.as_deref().unwrap_or("-"));
    println!("  {}  {}", label("Recording: "), if c.is_recording { "yes" } else { "no" });
    println!("  {}  {}", label("Dark:      "), if c.is_dark { "yes" } else { "no" });

    if let Some(hq) = c.hq_bytes_per_day {
        let lq = c.lq_bytes_per_day.unwrap_or(0);
        println!("  {}  {} HQ / {} LQ per day", label("Storage:   "), format_bytes(hq), format_bytes(lq));
    }
    ...
    if let Some(ref w) = c.wifi_connection_state {
        ...
        println!("    {}  {}", label("SSID:      "), w.ssid.as_deref().unwrap_or("-"));
        println!("    {}  {}", label("AP:        "), w.ap_name.as_deref().unwrap_or("-"));
        ...
        println!("    {}  {}", label("Status:    "), w.connectivity.as_deref().unwrap_or("-"));
    }
    ...
    if !c.channels.is_empty() {
        ...
        for ch in &c.channels {
            ...
            let fps = ch.fps.map(|f| format!("{f}fps")).unwrap_or_else(|| "-".into());
            let bitrate = ch.bitrate.map(|b| format!("{}kbps", b / 1000)).unwrap_or_else(|| "-".into());
            let rtsp = if ch.is_rtsp_enabled { "RTSP on" } else { "RTSP off" };
