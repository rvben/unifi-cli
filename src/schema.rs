...
fn command_metadata() -> HashMap<&'static str, CommandMeta> {
    ...
    let f = |fields: &'static [&'static str], mutating: bool, note: Option<&'static str>| {
        CommandMeta { output_fields: Some(fields), mutating, note }
    };
    let n = |note: &'static str| {
        CommandMeta { output_fields: None, mutating: false, note: Some(note) }
    };

    // clients
    m.insert("clients list",       f(&["name", "mac", "ip", "type"], false, None));
    m.insert("clients show",       f(&["name", "mac", "ip", "wired", "uptime", "tx_bytes", "rx_bytes", "signal", "ssid", "ap_mac"], false, None));
    m.insert("clients set-fixed-ip", f(&["status", "action", "mac", "ip", "name"], true, None));
    m.insert("clients block",      f(&["status", "action", "mac"], true, None));
    m.insert("clients unblock",    f(&["status", "action", "mac"], true, None));
    m.insert("clients kick",       f(&["status", "action", "mac"], true, None));
    m.insert("clients top",        f(&["name", "mac", "ip", "tx_bytes", "rx_bytes", "total_bytes"], false, None));

    // devices
    m.insert("devices list",       f(&["name", "model", "mac", "ip", "state", "firmware"], false, None));
    m.insert("devices show",       f(&["name", "model", "mac", "ip", "state", "firmware", "uptime", "num_sta", "version"], false, None));
    m.insert("devices restart",    f(&["status", "action", "mac"], true, None));
    m.insert("devices locate",     f(&["status", "action", "mac"], true, None));
    m.insert("devices ports",      f(&["port_idx", "name", "media", "up", "speed", "full_duplex", "poe_enable", "poe_power", "tx_bytes", "rx_bytes"], false, None));
    m.insert("devices upgrade",    f(&["status", "action", "mac"], true, None));

    // networks / events / system
    m.insert("networks",           f(&["name", "vlan_id", "enabled", "default"], false, None));
    m.insert("events list",        f(&["key", "msg", "subsystem", "time", "datetime"], false, None));
    m.insert("system health",      f(&["subsystem", "status", "num_sta", "num_ap", "num_switches", "wan_ip", "isp_name"], false, None));
    m.insert("system info",        f(&["hostname", "version", "timezone", "uptime"], false, None));

    // protect
    m.insert("protect cameras list", f(
        &["id", "name", "mac", "state", "model_key", "mic_enabled", "video_mode"], false,
        Some("With --full: adds ip, type, firmware, recording, resolution, codec, uptime, wifi"),
    ));
    ...
    m.insert("protect rtsps list",   f(&["high", "medium", "low", "package"], false, None));
    m.insert("protect rtsps create", f(&["status", "action", "camera_id", "streams"], true, None));
    m.insert("protect rtsps delete", f(&["status", "action", "camera_id", "qualities"], true, None));

    // utility commands
    m.insert("completions", n("Does not require --host or --api-key"));
    m.insert("config init",  n("Does not require --host or --api-key. Supports named profiles."));
    m.insert("config check", n("Requires --host and --api-key (or config file)."));
    m.insert("tui",          n("Interactive TUI. Keys: q quit, s sort, tab focus, / filter, up/down scroll"));
...
fn extract_global_flags(cmd: &clap::Command) -> serde_json::Map<String, serde_json::Value> {
    ...
    flags.insert("--json".into(), "Output as JSON (auto-enabled when piped)".into());
...
fn walk_commands(
    ...
    for sub in cmd.get_subcommands() {
        ...
        let children: Vec<_> = sub.get_subcommands().filter(|s| s.get_name() != "help").collect();
